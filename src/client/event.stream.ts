...
class EventStream extends EventEmitter {
  ...
  private readonly subscriptionResponseQueue = new Set<
    (subscription: EventStreamSubscribed) => void
  >();
  ...
  connect(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.connection && this.connection.readyState === WebSocket.OPEN) {
        this.emitter.emit(
          Events.DEBUG,
          `Open connection found, continuing operations.`,
          EventStream.label,
        );
      ...
      if (this.connection) {
        this.emitter.emit(
          Events.DEBUG,
          `Connection found, destroying connection.`,
          EventStream.label,
        );
  ...
  private onPackage(data: any): void {
    if (data.service === 'push') {
      switch (data.type) {
        ...
        default:
          throw new Error(
            `Received unknown push service: ${JSON.stringify(data)}`,
          );
      }
      ...
      switch (data.type) {
        ...
        default:
          throw new Error(
            `Received unknown event service: ${JSON.stringify(data)}`,
          );
      }
  ...
  private setHeartbeatTimer(interval: number): void {
    ...
    this.emitter.emit(
      Events.DEBUG,
      `Setting heartbeat interval(${interval}ms).`,
      EventStream.label,
    );
  ...
  private resetHeartbeat(): void {
    ...
    this.emitter.emit(
      Events.DEBUG,
      `Reset heartbeat acknowledgement.`,
      EventStream.label,
    );
  ...
  private updateSubscription(
    eventMessage: Record<string, any>,
  ): Promise<EventStreamSubscribed> {
    return new Promise((resolve, reject) => {
      if (!this.isReady)
        reject(
          new Error('Unable to update subscription to stream as it is closed'),
        );

      const closed = () => {
        this.subscriptionResponseQueue.delete(success);
        reject(
          new Error('Unable to update subscription to stream as it closed'),
        );
      };
