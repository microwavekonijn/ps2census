...
async function run() {
    ...
    await Promise.all(files.map(async file => {
        const {
            ...
            format,
            resolve
        } = require(join(COLLECTIONS_INPUT, file));

        const typeName = collection.replaceAll(/(?:^|_)([a-z0-9])/g, (_, s) => s.toUpperCase());
        ...
        // Resolve
        if (resolve)
            collectionType += `resolve: ${Object.keys(resolve).map(key => `'${key}'`).join(' | ')},`;
    ...
    index += collections.map((typeName) => `import {${typeName}} from './${typeName}';`).join('\n');
...
function buildFormat(format) {
    ...
    return `{${Object.entries(format).map(([k, v]) => `'${k}': ${buildFormat(v)}`).join(',')}}`
}
...
function getPaths(something, reducer) {
    ...
    return Object.entries(something).map(([key, something]) => reducer(key, getPaths(something, reducer))).flat();
}
